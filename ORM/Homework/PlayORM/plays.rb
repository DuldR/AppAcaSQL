...
class Playwright
    ...
    def get_plays
      raise "#{self} not in database" unless self.id
      data = PlayDBConnection.instance.execute("SELECT title, year, playwright_id, plays.id FROM plays JOIN playwrights ON plays.playwright_id = playwrights.id WHERE name = ?", self.name)
      data.map { |datum| Play.new(datum) }
    end
end